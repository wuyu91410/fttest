...
sample=function(a,pwr,ratio,ar){
	for (n in 2:1000){
		df1=n-1
		df2=round(ar*df1)
		if(df2 < 1) df2=1
...
shinyServer(function(input, output) {
        output$text1 <- renderText({
            paste("Using the Contractor's Risk,",paste(tags$i(HTML("&alpha;")),",",sep=""),"of",input$a1,"with the sample size given, the risk of the State DOT not detecting a difference between the variances of State DOT's and Contractor's sample data when one exists,",paste(tags$i(HTML("&beta;")),",",sep=""),"is",paste(tags$code(1-round(pwr(input$var1,input$var2,input$n1,input$n2,as.numeric(input$a1)),4)),".",sep=""),"Thus the power of the test, (1-",HTML(paste(tags$i(HTML("&beta;")),")",sep="")),"is",paste(tags$code(round(pwr(input$var1,input$var2,input$n1,input$n2,as.numeric(input$a1)),4)),".",sep=""),"
        ...
        output$text2<- renderText({
        	paste("Using the Contractor's Risk,",paste(tags$i(HTML("&alpha;")),",",sep=""),"of",input$a1,"with the sample size given, the risk of the State DOT not detecting a difference between the means of State DOT and Contractor's sample data when one exists ",tags$i(HTML("&beta;")),"is",paste(tags$code(1-round(pwr.t2n.test(input$n1,input$n2,d=d(input$mu1,input$mu2,input$var1,input$var2,input$n1,input$n2),as.numeric(input$a1))$power,4)),".",sep=""),"Thus the power of the test, (1-",HTML(paste(tags$i(HTML("&beta;")),")",sep="")),"is",paste(tags$code(round(pwr.t2n.test(input$n1,input$n2,d=d(input$mu1,input$mu2,input$var1,input$var2,input$n1,input$n2),as.numeric(input$a1))$power,4)),".",sep="")
        ...
        output$text5<- renderUI({
        	if (input$ar != 1){
        		...
        		str1 <- paste("The required sample size for State DOT data is",tags$code(sample(as.numeric(input$a2),input$pwr,input$var3/input$var4,input$ar)[1]))
				str2 <- paste("The required sample size for Contractor data is",tags$code(sample(as.numeric(input$a2),input$pwr,input$var3/input$var4,input$ar)[2]))
				HTML(paste(str,str1,str2,sep="<br>"))
        					...
        					using equal sample sized, the required sample size for each would be",tags$code(sample(as.numeric(input$a2),input$pwr,input$var3/input$var4,input$ar)[1]))
				HTML(paste(str))
				}
        })
        ...
        output$text6 <- renderUI({
        	...
        	str1 <- paste("The required sample size for the State DOT data is",tags$code(n.ttest(power=input$pwr,alpha=as.numeric(input$a2),mean.diff=input$mu3-input$mu4,
        																 sd1=sqrt(input$var3),sd2=sqrt(input$var4),k=input$ar,fraction="unbalanced")[[2]]))
        	str2 <- paste("The required sample size for the Contractor data is",tags$code(n.ttest(power=input$pwr,alpha=as.numeric(input$a2),mean.diff=input$mu3-input$mu4,
        																 sd1=sqrt(input$var3),sd2=sqrt(input$var4),k=input$ar,fraction="unbalanced")[[3]]))
			HTML(paste(str,str1,str2,sep="<br>"))   
        	}
        ...
        output$text7 <- renderUI({
        	...
        	str6<- paste("The optimized sample size of",paste("n",tags$sub("State."),sep=""),"=", sample(as.numeric(input$a2),input$pwr,input$var3/input$var4,input$ar)[1])
        	str7<- paste("The optimized sample size of",paste("n",tags$sub("Contr."),sep=""),"=",sample(as.numeric(input$a2),input$pwr,input$var3/input$var4,input$ar)[2])
        	HTML(paste(str,str1,str2,str3,str4,str5,str6,str7,sep="<br>"))
        })
